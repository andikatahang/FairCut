...
export default function RecruitmentPage() {
  ...
  const applicantsForJob = mockApplicants.filter(a => a.job_id === selectedJob && !['rejected','offer_expired'].includes(a.tahap))
                ...
                return (
                  <div key={stage} className="flex-1 min-w-[160px]">
                    ...
                    <div className="space-y-3">
                      {cards.map(a => (
                        <div key={a.applicant_id} className="card-sm cursor-pointer hover:shadow-card-md hover:border-navy/20 transition-all">
                      ...
                      {cards.length === 0 && (
                        <div className="border-2 border-dashed border-border rounded-xl p-4 text-center">
                          <p className="text-xs text-navy/30">Empty</p>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        {dssModal && (
          <div className="space-y-4">
            ...
            <div className="flex gap-3 mt-2">
              <button className="btn-primary flex-1 justify-center">Confirm: Photo Retouching</button>
              <button className="btn-secondary flex-1 justify-center" onClick={() => setDssModal(null)}>Cancel</button>
            </div>
          </div>
        )}
      </Modal>
